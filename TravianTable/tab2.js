...
buttonOpen = () => {//действие по нажатию на кнопку
	...
	for (let i = 0; i < spisokID.length; i++) {
			...
			//console.log(deltaCoordx);
			//console.log(deltaCoordy);
			//console.log(spisokOasisFull[i].x);
			//console.log(spisokOasisFull[i].y);
	...
	for (let i = 0; i < spisokOasisFull.length; i++) {
		qwert1 = window.MapDetails.get(spisokOasisFull[i].ID).data.troops.units[1];
		qwert2 = window.MapDetails.get(spisokOasisFull[i].ID).data.troops.units[2];
		qwert3 = window.MapDetails.get(spisokOasisFull[i].ID).data.troops.units[3];
		qwert4 = window.MapDetails.get(spisokOasisFull[i].ID).data.troops.units[4];
		qwert5 = window.MapDetails.get(spisokOasisFull[i].ID).data.troops.units[5];
		qwert6 = window.MapDetails.get(spisokOasisFull[i].ID).data.troops.units[6];
		qwert7 = window.MapDetails.get(spisokOasisFull[i].ID).data.troops.units[7];
		qwert8 = window.MapDetails.get(spisokOasisFull[i].ID).data.troops.units[8];
		qwert9 = window.MapDetails.get(spisokOasisFull[i].ID).data.troops.units[9];
		qwert10 = window.MapDetails.get(spisokOasisFull[i].ID).data.troops.units[10];
		if (qwert1 !== undefined) {let b = spisokOasisFull[i];spisokOasisUnits1.push(b);/*запишем ее в список оазисов с нужными юнитами*/}
		if (qwert2 !== undefined){let b = spisokOasisFull[i];spisokOasisUnits2.push(b);}
		if (qwert3 !== undefined){let b = spisokOasisFull[i];spisokOasisUnits3.push(b);}
		if (qwert4 !== undefined){let b = spisokOasisFull[i];spisokOasisUnits4.push(b);}
		if (qwert5 !== undefined){let b = spisokOasisFull[i];spisokOasisUnits5.push(b);}
		if (qwert6 !== undefined){let b = spisokOasisFull[i];spisokOasisUnits6.push(b);}
		if (qwert7 !== undefined){let b = spisokOasisFull[i];spisokOasisUnits7.push(b);}
		if (qwert8 !== undefined){let b = spisokOasisFull[i];spisokOasisUnits8.push(b);}
		if (qwert9 !== undefined){let b = spisokOasisFull[i];spisokOasisUnits9.push(b);}
		if (qwert10 !== undefined){let b = spisokOasisFull[i];spisokOasisUnits10.push(b);}
	}

	//для вывода в таблицу списка оазисов.
	for (let i = 0; i < spisokOasisUnits1.length; i++) {
		vTablet ='. . . . . . . . . ' + window.MapDetails.get(spisokOasisUnits1[i].ID).data.troops.units[1] + '  ' + oasis_date[1] + ' в  ' + window.Village.get(spisokOasisUnits1[i].ID).data.name + '. . - → . ' + spisokOasisUnits1[i].dist + '. . . . . . . . . . .';
		spisokVTablet1.push(vTablet);
	}
	for (let i = 0; i < spisokOasisUnits2.length; i++) {
		vTablet ='. . . . . . . . . . .' + window.MapDetails.get(spisokOasisUnits2[i].ID).data.troops.units[2] + '  ' + oasis_date[2] + ' в  ' + window.Village.get(spisokOasisUnits2[i].ID).data.name + '. . - → . ' + spisokOasisUnits2[i].dist + '. . . . . . . . . .';
		spisokVTablet2.push(vTablet);
	}
	for (let i = 0; i < spisokOasisUnits3.length; i++) {
		vTablet ='. . . . . . . . . . .' + window.MapDetails.get(spisokOasisUnits3[i].ID).data.troops.units[3] + '  ' + oasis_date[3] + ' в  ' + window.Village.get(spisokOasisUnits3[i].ID).data.name + '. . - → . ' + spisokOasisUnits3[i].dist + '. . . . . . . . . .';
		spisokVTablet3.push(vTablet);
	}
	for (let i = 0; i < spisokOasisUnits4.length; i++) {
		vTablet ='. . . . . . ' + window.MapDetails.get(spisokOasisUnits4[i].ID).data.troops.units[4] + '  ' + oasis_date[4] + ' в  ' + window.Village.get(spisokOasisUnits4[i].ID).data.name + '. . - → . ' + spisokOasisUnits4[i].dist+ '. . . . . . .';
		spisokVTablet4.push(vTablet);
	}
	for (let i = 0; i < spisokOasisUnits5.length; i++) {
		vTablet ='. . . . . . . . . .' + window.MapDetails.get(spisokOasisUnits5[i].ID).data.troops.units[5] + '  ' + oasis_date[5] + ' в  ' + window.Village.get(spisokOasisUnits5[i].ID).data.name + '. . - → . ' + spisokOasisUnits5[i].dist+ '. . . . . . . . . .';
		spisokVTablet5.push(vTablet);
	}
	for (let i = 0; i < spisokOasisUnits6.length; i++) {
		vTablet ='. . . . . . . . . . . .' + window.MapDetails.get(spisokOasisUnits6[i].ID).data.troops.units[6] + '  ' + oasis_date[6] + ' в  ' + window.Village.get(spisokOasisUnits6[i].ID).data.name + '. . - → . ' + spisokOasisUnits6[i].dist + '. . . . . . . . . .';
		spisokVTablet6.push(vTablet);
	}
	for (let i = 0; i < spisokOasisUnits7.length; i++) {
		vTablet = '. . . . . . . . .' +window.MapDetails.get(spisokOasisUnits7[i].ID).data.troops.units[7] + '  ' + oasis_date[7] + ' в  ' + window.Village.get(spisokOasisUnits7[i].ID).data.name + '. . - → . ' + spisokOasisUnits7[i].dist+ '. . . . . . . . . .';
		spisokVTablet7.push(vTablet);
	}
	for (let i = 0; i < spisokOasisUnits8.length; i++) {
		vTablet = '. . . . . . . . .' +window.MapDetails.get(spisokOasisUnits8[i].ID).data.troops.units[8] + '  ' + oasis_date[8] + ' в  ' + window.Village.get(spisokOasisUnits8[i].ID).data.name + '. . - → . ' + spisokOasisUnits8[i].dist+ '. . . . . . . . . .';
		spisokVTablet8.push(vTablet);
	}
	for (let i = 0; i < spisokOasisUnits9.length; i++) {
		vTablet = '. . . . . . . . . . .' +window.MapDetails.get(spisokOasisUnits9[i].ID).data.troops.units[9] + '  ' + oasis_date[9] + ' в  ' + window.Village.get(spisokOasisUnits9[i].ID).data.name + '. . - → . ' + spisokOasisUnits9[i].dist+ '. . . . . . . . . .';
		spisokVTablet9.push(vTablet);
	}
	for (let i = 0; i < spisokOasisUnits10.length; i++) {
		vTablet ='. . . . . . . . . .' + window.MapDetails.get(spisokOasisUnits10[i].ID).data.troops.units[10] + '  ' + oasis_date[10] + ' в  ' + window.Village.get(spisokOasisUnits10[i].ID).data.name + '. . - → . ' + spisokOasisUnits10[i].dist+ '. . . . . . . . . .';
	...
	document.getElementById("testDataVT1").textContent = spisokVTablet1.sort();//список оазисов с мышками
	document.getElementById("testDataVT2").textContent = spisokVTablet2.sort();//sort()-для сортировки А-Я
	document.getElementById("testDataVT3").textContent = spisokVTablet3.sort();//reverse()-для сортировки Я-А
	document.getElementById("testDataVT4").textContent = spisokVTablet4.sort();//slice()-создает новый список???
	document.getElementById("testDataVT5").textContent = spisokVTablet5.sort();
	document.getElementById("testDataVT6").textContent = spisokVTablet6.sort();
	document.getElementById("testDataVT7").textContent = spisokVTablet7.sort();
	document.getElementById("testDataVT8").textContent = spisokVTablet8.sort();
	document.getElementById("testDataVT9").textContent = spisokVTablet9.sort();
	document.getElementById("testDataVT10").textContent = spisokVTablet10.sort();

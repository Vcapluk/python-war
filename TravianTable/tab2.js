//2.вывод ЧВР, ЕК.
	...
	function Never_MainScript() {
		...
		NavBar = () => {
			...
			str +=			'<li class="tab-content__item js-tab-content active" data-tab="1">';
					str += 'строка перед таблицей'
			str +=				'<table><tr><td>ЧВР</td><td><span id="testData"></span></td>';
			str +=					'<tr><td>Сундук 3%</td><td><span id="testData3"></span></td>';
			str +=					'<tr><td>Сундук 4%</td><td><span id="testData4"></span></td>';
			str +=					'<tr><td>Сундук 5%</td><td><span id="testData5"></span></td>';
					str += '</table>'
			str +=			'</li>';
	...
	buttonOpen = () => {//действие по нажатию на кнопку
		...
		const sumRes = spisokCHVRRes.reduce((partialSum, a) => partialSum + a, 0);
		//console.log(sum);
		//console.log("ЧВР по другому методу равно: " + sum);
		
		//для вывода на экран
		const sumCHVR = sum.toString();
		const sumCHVRRes = sumRes.toString();
		
		const sumCHVR3 = Math.ceil(sumCHVRRes*24*0.03);
		const sumCHVR4 = Math.ceil(sumCHVRRes*24*0.04);
		const sumCHVR5 = Math.ceil(sumCHVRRes*24*0.05);
		document.getElementById("testData").textContent = sumCHVR;
		document.getElementById("testData3").textContent = sumCHVR3;
		document.getElementById("testData4").textContent = sumCHVR4;
		document.getElementById("testData5").textContent = sumCHVR5;

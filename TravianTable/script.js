...
//Этот код представляет собой цикл, который добавляет обработчик события клика ко всем элементам массива buttons. Каждый элемент массива
// предполагается быть кнопкой на веб-странице.
...
//Этот код представляет собой цикл, который просматривает все элементы с тегом <span> и назначает каждому из них обработчик события
// onclick. Когда происходит клик по элементу <span>, выполняется функция, которая скрывает модальное окно.
for (var i = 0; i < span.length; i++) {//Цикл проходится по всем элементам с тегом <span>. span предполагается, что это 
                                        //коллекция или массив элементов с тегом <span>. В каждой итерации цикла переменная
                                        // i используется для доступа к текущему элементу в коллекции.
  span[i].onclick = function() {//Для каждого элемента <span> устанавливается обработчик события onclick. Когда происходит клик по 
                                // элементу, выполняется анонимная функция.
    var id = this.getAttribute('data-modal');//Внутри функции определяется переменная id, которая получает значение атрибута data-modal
                                                // текущего элемента, по которому был сделан клик. Предполагается, что у элемента 
                                                // есть атрибут data-modal,который содержит идентификатор модального окна.
...
//Этот код добавляет обработчик события клика (onclick) к объекту window в веб-странице. Когда происходит клик в любом месте
// окна (window), код выполняет следующие действия:
window.onclick = function(event) {//Получает объект события event.
  var isModal = (' ' + event.target.className + ' ').indexOf(' modal ') > -1;//Извлекает класс элемента, по которому произошел клик, 
                                                                            //с использованием event.target.className
